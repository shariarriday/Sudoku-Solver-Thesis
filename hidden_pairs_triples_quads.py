...
def hidden_remove(rowcolbox,group_no,hidden_inxs,comb,cands,square_pos,no_of_hidden):
    ...
    for cells in hidden_inxs:
        ...
        if len(removed_vals):
            cands.at[row,col] = np.array(list(set(comb).intersection(set(cands.iloc[row,col]))))
            f = open('Output/log.txt','a')
            f.write(f"R{row:<1}C{col:<1}   :  Hidden {pairtriplequad[no_of_hidden]}s ({rowcolbox:<3}), {str(removed_vals):<15} removed, {pairtriplequad[no_of_hidden]:>7}s: {str(comb):>6}\n")
            print(f"R{row:<1}C{col:<1}   :  Hidden {pairtriplequad[no_of_hidden]}s ({rowcolbox:<3}), {str(removed_vals):<15} removed, {pairtriplequad[no_of_hidden]:>7}s: {str(comb):>6}")
            f.close()
            ischanged = 1
