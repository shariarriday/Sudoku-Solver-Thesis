...
def naked_remove(rowcolbox,group_no,naked_inxs,comb,cands,square_pos,no_of_nakeds):
    ...
    for cells in naked_inxs:
        ...
        if len(removed_vals):
            ...
            f.write(f"R{row:<1}C{col:<1}   :  Naked {pairtriplequad[no_of_nakeds]}s ({rowcolbox:<3}), {str(removed_vals):<15} removed, {pairtriplequad[no_of_nakeds]:>7}s: {str(comb):>6}\n")
            print(f"R{row:<1}C{col:<1}   : Naked {pairtriplequad[no_of_nakeds]}s ({rowcolbox:<3}), {str(removed_vals):<15} removed, {pairtriplequad[no_of_nakeds]:>7}s: {str(comb):>6}")
            f.close()
            ischanged = 1
