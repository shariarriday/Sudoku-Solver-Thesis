        ...
        for valun in valco.index[valco == 1].values: #????? there can be 1 and only 1 single candidate in a row..
            ...
            if len(inx):
                ...
                for inxt in inx:
                    ...
                    f.write(f"R{row+1}C{inxt+1}={valun} : Hidden Singles (row)\n")
    ...
    for col in range(9):
        ...
        for valun in valco.index[valco == 1].values:
            ...
            if len(inx):
                ...
                for inxt in inx:
                    f = open('Output/log.txt','a')
                    f.write(f"R{inxt+1}C{col+1}={valun} : Hidden Singles (col)\n")
                            ...
                            try: 
                                if board.iloc[rowx,colx] == ".":
                                    if to_change in a.loc[rowx][colx]:
                                        f = open('Output/log.txt','a')
                                        f.write(f"R{rowx+1}C{colx+1}={to_change} : Hidden Singles (square)\n")
